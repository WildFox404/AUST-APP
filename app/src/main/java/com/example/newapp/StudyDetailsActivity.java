...
public class StudyDetailsActivity extends AppCompatActivity {
    ...
    private void initUI() {
        ...
        ArrayList<ArrayList<Item>> item_list=new ArrayList<>();
        for (JsonElement jsonArrayData : jsonArrayDatas) {
            ...
            ArrayList<Item> item_list_child=new ArrayList<>();
            for (JsonElement plan_course_group_audit_result : plan_course_group_audit_results) {
                ...
                Item item = new Item(group_course_type_name,group_credits_completed,group_credits_required,group_num_completed,group_num_required,group_plan_course_audit_string_results);
                item_list_child.add(item);
            }
            item_list.add(item_list_child);
            Grade grade =new Grade(course_type_name,credits_completed,credits_required,num_completed,num_required,relation_text);
            grade_list.add(grade);
        }
        if (grade_list.size() == 0) {
            //无记录
        } else {
            study_details_expandablelistview.setAdapter(new GradeAdapter(this, grade_list,item_list));
        }
    }
}
